...
{
    public partial class frmGerenciarStatusPagamento : Form
    {
        private StatusPagamentoCtr controller;
        private StatusPagamento instancia;

        public frmGerenciarStatusPagamento(StatusPagamentoCtr ctr)
        {
            InitializeComponent();
        ...
        {
            dgvStatusPagamento.DataSource = resultado;
            dgvStatusPagamento.Refresh();
        }
        ...
        private void dgvFormasPagamento_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string tipoSelecionado = dgvStatusPagamento.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (!string.IsNullOrEmpty(tipoSelecionado))
                {
                    instancia.Id = Convert.ToInt32(tipoSelecionado);
                    instancia.Descricao = dgvStatusPagamento.Rows[e.RowIndex].Cells[1].Value.ToString();
