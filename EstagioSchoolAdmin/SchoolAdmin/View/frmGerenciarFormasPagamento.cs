...
{
    public partial class frmGerenciarFormasPagamento : Form
    {
        private FormasPagamentoCtr controller;
        private FormaPagamento instancia;

        public frmGerenciarFormasPagamento(FormasPagamentoCtr ctr)
        {
            InitializeComponent();
        ...
        {
            dgvFormasPagamento.DataSource = resultado;
        ...
        private void dgvTipos_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string tipoSelecionado = dgvFormasPagamento.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (!string.IsNullOrEmpty(tipoSelecionado))
                {
                    instancia.Id = Convert.ToInt32(tipoSelecionado);
                    instancia.Descricao = dgvFormasPagamento.Rows[e.RowIndex].Cells[1].Value.ToString();
