...
{
    public partial class frmGerenciarNecessidadesEspeciais : Form
    {
        private NecessidadesCtr controller;
        private NecessidadeEspecial instancia;

        public frmGerenciarNecessidadesEspeciais(NecessidadesCtr ctr)
        {
            InitializeComponent();
        ...
        {
            dgvNecessidades.DataSource = resultado;
            dgvNecessidades.Refresh();
        }
        ...
        private void dgvMedicamentos_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string tipoSelecionado = dgvNecessidades.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (!string.IsNullOrEmpty(tipoSelecionado))
                {
                    instancia.Id = Convert.ToInt32(tipoSelecionado);
                    instancia.Descricao = dgvNecessidades.Rows[e.RowIndex].Cells[1].Value.ToString();
