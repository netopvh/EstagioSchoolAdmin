...
{
    public partial class frmGerenciarDefiencias : Form
    {
        private DeficienciasCtr controller;
        private Deficiencia instancia;

        public frmGerenciarDefiencias(DeficienciasCtr ctr)
        {
            InitializeComponent();
        ...
        {
            dgvDeficiencias.DataSource = resultado;
            dgvDeficiencias.Refresh();
        }
        ...
        private void dgvDeficiencias_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string tipoSelecionado = dgvDeficiencias.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (!string.IsNullOrEmpty(tipoSelecionado))
                {
                    instancia.Id = Convert.ToInt32(tipoSelecionado);
                    instancia.Descricao = dgvDeficiencias.Rows[e.RowIndex].Cells[1].Value.ToString();
