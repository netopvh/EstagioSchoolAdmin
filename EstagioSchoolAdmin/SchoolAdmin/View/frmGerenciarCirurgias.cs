...
{
    public partial class frmGerenciarCirurgias : Form
    {
        private CirurgiasCtr controller;
        private Cirurgia instancia;

        public frmGerenciarCirurgias(CirurgiasCtr ctr)
        {
            InitializeComponent();
            controller = ctr;
        ...
        {
            dgvCirurgias.DataSource = resultado;
            dgvCirurgias.Refresh();
        }
        ...
        private void dgvStatusPagamento_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string tipoSelecionado = dgvCirurgias.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (!string.IsNullOrEmpty(tipoSelecionado))
                {
                    instancia.Id = Convert.ToInt32(tipoSelecionado);
                    instancia.Descricao = dgvCirurgias.Rows[e.RowIndex].Cells[1].Value.ToString();
