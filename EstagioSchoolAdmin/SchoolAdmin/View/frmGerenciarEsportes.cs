...
{
    public partial class frmGerenciarEsportes : Form
    {
        private EsporteCtr controller;
        private Esporte instancia;

        public frmGerenciarEsportes(EsporteCtr ctr)
        {
            InitializeComponent();
        ...
        {
            dgvEsportes.DataSource = resultado;
            dgvEsportes.Refresh();
        }
        ...
        private void dgvEsportes_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string tipoSelecionado = dgvEsportes.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (!string.IsNullOrEmpty(tipoSelecionado))
                {
                    instancia.Id = Convert.ToInt32(tipoSelecionado);
                    instancia.Descricao = dgvEsportes.Rows[e.RowIndex].Cells[1].Value.ToString();
