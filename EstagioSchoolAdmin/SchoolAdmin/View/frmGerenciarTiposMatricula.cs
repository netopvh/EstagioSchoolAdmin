...
{
    public partial class frmGerenciarTiposMatricula : Form
    {
        private TipoMatriculaCtr controller;
        private TipoMatricula instancia;

        public frmGerenciarTiposMatricula(TipoMatriculaCtr ctr)
        {
            InitializeComponent();
        ...
        private void dgvTipos_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string tipoSelecionado = dgvTipos.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (!string.IsNullOrEmpty(tipoSelecionado))
                {
                    instancia.Id = Convert.ToInt32(tipoSelecionado);
                    instancia.Descricao = dgvTipos.Rows[e.RowIndex].Cells[1].Value.ToString();
