...
{
    public partial class frmGerenciarTratamentosSaude : Form
    {
        private TratamentoSaudeCtr controller;
        private TratamentoSaude instancia;

        public frmGerenciarTratamentosSaude(TratamentoSaudeCtr ctr)
        {
            InitializeComponent();
        ...
        {
            dgvTratamentos.DataSource = resultado;
            dgvTratamentos.Refresh();
        }
        ...
        private void dgvTratamentos_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string tipoSelecionado = dgvTratamentos.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (!string.IsNullOrEmpty(tipoSelecionado))
                {
                    instancia.Id = Convert.ToInt32(tipoSelecionado);
                    instancia.Descricao = dgvTratamentos.Rows[e.RowIndex].Cells[1].Value.ToString();
