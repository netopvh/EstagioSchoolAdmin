...
{
    public partial class frmGerenciarFraturas : Form
    {
        private FraturaCtr controller;
        private Fratura instancia;

        public frmGerenciarFraturas(FraturaCtr ctr)
        {
            InitializeComponent();
        ...
        {
            dgvFraturas.DataSource = resultado;
            dgvFraturas.Refresh();
        }
        ...
        private void dgvFraturas_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string tipoSelecionado = dgvFraturas.Rows[e.RowIndex].Cells[0].Value.ToString();
                if (!string.IsNullOrEmpty(tipoSelecionado))
                {
                    instancia.Id = Convert.ToInt32(tipoSelecionado);
                    instancia.Descricao = dgvFraturas.Rows[e.RowIndex].Cells[1].Value.ToString();
